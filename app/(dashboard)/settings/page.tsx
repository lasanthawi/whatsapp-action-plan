...
type SettingsSection = 'profile' | 'agent' | 'automation' | 'account';

type PageProps = {
  searchParams?: { updated?: string; section?: string };
};

function getActiveSection(section?: string): SettingsSection {
  if (section === 'agent' || section === 'automation' || section === 'account') {
    return section;
  }
  return 'profile';
}
...
export default async function SettingsPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="mobile-page mobile-page-settings">
      ...
      <nav className="mobile-settings-tabs" aria-label="Settings sections">
        {tabs.map((tab) => (
          <Link
            key={tab.id}
            href={`/settings?section=${tab.id}`}
            className={`mobile-settings-tab ${activeSection === tab.id ? 'mobile-settings-tab-active' : ''}`}
            scroll={false}
          >
            {tab.label}
          </Link>
